...
if __name__ == '__main__':
    ...
    test(re_a, '555', False)
    test(re_a, '2312351231231235', True)

    re_b = re.compile(hw6.b)
    test(re_b, '1:45 PM', True)
    test(re_b, '0:59 AM', False)
    test(re_b, '1:19 AM', True)
    test(re_b, '06:59 AM', False)
    ...
    test(re_c, ', hello', False)
    test(re_c, '8y, hello', False)
    test(re_c, 'y, z, ', False)
    ...
    test(re_d, '< b', False)
    test(re_d, '2 <= b', True)
    test(re_d, 'test1               <=     btest2', True)
    test(re_d, 'adsfasdfasdf a < b', False)
    ...
    print('part d test 3')
    result = re_d.sub(hw6.d_sub, 'wqeuqw<wioejoqiwje || 231     <=     3231')
    expected = 'wioejoqiwje > wqeuqw || 3231 >= 231'
    if result == expected:
        print('passed')
    else:
        print("FAILED\nexpected: '{}'\ngot:      '{}'".format(expected, result))
