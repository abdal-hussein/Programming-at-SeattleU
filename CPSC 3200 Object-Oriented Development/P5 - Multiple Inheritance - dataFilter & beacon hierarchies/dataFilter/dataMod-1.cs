...
namespace P5
{
    ...
    {
        private const uint REPLACE_PRIMES_WITH = 2;

        public dataMod(uint inPrime, uint[] seq = null) : base(inPrime, seq) 
            { type = 1; }
        ...
        public override uint[] filter()
        {
            ...
            if (!(filteredSeq.Length == 1 && filteredSeq[0] == prime))
                for (int i = 0; i < filteredSeq.Length; i++)
                    filteredSeq[i] = largeMode ? filteredSeq[i] + 1
                        : filteredSeq[i] - 1;
            return filteredSeq;
        }
